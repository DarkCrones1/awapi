...
public class CultureRepository : CatalogBaseRepository<Culture>, ICultureRepository
{
    public CultureRepository(AWDbContext dbContext) : base(dbContext)
    {
    }

    public override async Task<IEnumerable<Culture>> GetPaged(Culture entity)
    {
        var query = _dbContext.Culture.AsQueryable();

        return await query.ToListAsync();
    }

    public async Task<IEnumerable<Culture>> GetPaged(CultureQueryFilter entity)
    {
        var query = _dbContext.Culture.AsQueryable();
